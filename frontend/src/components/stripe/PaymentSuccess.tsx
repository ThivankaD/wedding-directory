...
export default function PaymentSuccess({ searchParams }: PageProps) {
  ...
  useEffect(() => {
    if (payment?.status === 'completed' && urlSearchParams.get('print') === 'true') {
  ...
  return (
    <>
      ...
      {/* ============================================================ */}
      <div className="py-12 px-4 max-w-2xl mx-auto print:hidden">
        <div className="bg-white rounded-3xl shadow-sm border border-amber-100/60 overflow-hidden">
      ...
      {payment && (
        <div className="hidden print:block font-sans text-gray-900 bg-white p-6 max-w-2xl mx-auto border border-gray-300 rounded-2xl print-avoid-break">
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-3 text-xs py-2">
            <div>
              <span className="text-gray-400 text-[11px] block font-medium">Vendor / Service Provider</span>
              <span className="font-semibold text-gray-900 text-sm">{payment.vendorName || 'Wedding Vendor'}</span>
            </div>

            <div>
              <span className="text-gray-400 text-[11px] block font-medium">Service &amp; Package</span>
            ...
            <div>
              <span className="text-gray-400 text-[11px] block font-medium">Booked Event Date</span>
            ...
            <div>
              <span className="text-gray-400 text-[11px] block font-medium">Payment Gateway</span>
            ...
            <div>
              <span className="text-gray-400 text-[11px] block font-medium">Order Reference</span>
              <span className="font-mono text-xs font-semibold text-gray-800">{payment.orderId}</span>
            </div>

            {payment.gatewayPaymentId && (
              <div>
                <span className="text-gray-400 text-[11px] block font-medium">PayHere Payment ID</span>
                <span className="font-mono text-xs font-semibold text-gray-800">{payment.gatewayPaymentId}</span>
              </div>
            )}

            {payment.customerEmail && (
              <div>
                <span className="text-gray-400 text-[11px] block font-medium">Customer Email</span>
                <span className="text-xs text-gray-800">{payment.customerEmail}</span>
              </div>
            )}
          </div>
          ...
          <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
            <table className="w-full text-left text-xs">
                ...
                <tr>
                  <td className="py-3 px-3">
                    ...
                    <p className="text-[11px] text-gray-500">
                      Package: {payment.packageName || 'Standard'} &bull; Event Date:{' '}
                      {payment.bookingDate
                        ? new Date(payment.bookingDate).toLocaleDateString('en-US', {
                            month: 'short',
                            day: 'numeric',
              ...
              <tfoot className="bg-gray-50/90 border-t border-gray-200 font-semibold text-xs">
                <tr>
                  <td colSpan={2} className="py-2.5 px-3 text-right text-gray-600">
                    Total Advance Paid:
                  </td>
          ...
          <div className="mt-4 p-3 bg-amber-50/70 border border-amber-200/60 rounded-lg text-[11px] text-amber-900 space-y-1">
            ...
            <p className="text-amber-800 leading-relaxed">
              This receipt confirms your advance booking deposit. Your vendor has been notified and will coordinate wedding day arrangements directly with you. The remaining balance is payable according to the vendor&apos;s agreed schedule.
            </p>
          </div>

          {/* Receipt Footer */}
          <div className="mt-5 pt-3 border-t border-gray-200 flex items-center justify-between text-[10px] text-gray-400">
            <span>Say I Do &copy; {new Date().getFullYear()} &bull; All Rights Reserved</span>
            <span>Computer-generated electronic receipt &bull; No signature required</span>
          </div>
        </div>
      )}
    </>
