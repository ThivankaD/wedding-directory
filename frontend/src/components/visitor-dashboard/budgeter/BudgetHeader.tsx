...
const BudgetHeader = ({ budget = 0.00, totalCost = 0.00 }) => {
  ...
  return (
    <div className="bg-white rounded-3xl border-2 border-orange/20 shadow-sm p-6 sm:p-8 relative overflow-hidden w-full">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="font-body text-gray-600 text-sm sm:text-base mt-1.5 max-w-xl">
            Plan your wedding expenses effectively, track payments, and stay comfortably within budget.
          </p>
        </div>
        <div className="flex flex-col items-start md:items-end bg-orange/[0.05] border-2 border-orange/20 rounded-2xl p-4 sm:p-5 shrink-0 min-w-[200px]">
          <p className="font-body text-xs font-bold uppercase tracking-wider text-gray-500">Budget Utilized</p>
          <p className="font-title text-3xl sm:text-4xl font-bold text-orange my-0.5">{utilizationPercentage}%</p>
          <p className="font-body text-xs text-gray-500">of your total budget</p>
        </div>
      </div>
    </div>
  );
