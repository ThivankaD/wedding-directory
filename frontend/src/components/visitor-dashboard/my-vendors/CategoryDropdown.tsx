  ...
  return (
    <button
      ...
      className={`group relative w-full text-left rounded-2xl border-2 transition-all duration-300 p-5 sm:p-6 flex flex-col justify-between hover:-translate-y-1 hover:shadow-lg focus:outline-none focus:ring-2 focus:ring-orange/30 cursor-pointer ${
        hasVendors
          ? 'bg-orange/[0.03] border-orange/35 hover:border-orange shadow-xs'
          : 'bg-white border-orange/20 hover:border-orange shadow-2xs'
      }`}
    >
      <div>
        {/* Top Row: Icon + Badge */}
        <div className="flex items-center justify-between gap-3">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 ${
              hasVendors
                ? 'bg-orange/15 text-orange shadow-2xs'
                : 'bg-orange/[0.08] text-orange/80 group-hover:bg-orange/15 group-hover:text-orange'
            }`}
          >
          ...
          <span
            className={`text-xs font-bold px-3 py-1 rounded-full transition-colors ${
              hasVendors
                ...
                : 'bg-gray-100 text-gray-500 group-hover:bg-orange/10 group-hover:text-orange'
            }`}
          >
        ...
        <h3 className="font-title text-lg sm:text-xl font-bold text-gray-900 group-hover:text-orange transition-colors mt-4 mb-1.5">
          {category}
        </h3>
      ...
      <div className="flex items-center justify-between pt-3.5 mt-4 border-t border-orange/15 text-xs font-semibold">
        <span
          className={`transition-colors ${
            hasVendors ? 'text-orange font-bold' : 'text-gray-500 group-hover:text-orange'
          }`}
        >
...
export const CategoryModal: React.FC<CategoryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal Container */}
      <div className="bg-white rounded-3xl shadow-2xl border border-orange/20 w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden relative z-10 animate-in fade-in zoom-in-95 duration-200">
        {/* Modal Header */}
        <div className="flex items-center justify-between p-5 sm:p-6 border-b border-orange/10 bg-orange/[0.03]">
          <div className="flex items-center gap-3.5 min-w-0">
            <div className="w-12 h-12 rounded-2xl bg-orange/15 text-orange flex items-center justify-center shrink-0 shadow-2xs">
              <IconComponent size={22} />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h2 className="font-title text-xl sm:text-2xl font-bold text-gray-900 truncate">
                  {category}
                </h2>
                <span
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-9 h-9 rounded-full bg-white border border-gray-200 text-gray-500 hover:text-orange hover:border-orange/30 hover:bg-orange/10 flex items-center justify-center transition-all shrink-0 ml-2 cursor-pointer"
          >
            <FiX size={18} />
          </button>
        </div>
        ...
        {/* Modal Body */}
        <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-3 bg-orange/[0.01]">
          {loading ? (
            ...
            <div className="text-center py-10 px-4 bg-white rounded-2xl border border-dashed border-orange/25 my-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500 mt-1 mb-5 max-w-sm mx-auto">
        ...
        {/* Modal Footer */}
        <div className="p-4 sm:p-5 border-t border-orange/10 bg-white flex flex-col sm:flex-row items-center justify-between gap-3">
          <Link
            href={`/vendor-search?category=${encodeURIComponent(category)}`}
            onClick={onClose}
          ...
          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto px-5 py-2 rounded-xl text-xs font-semibold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-all cursor-pointer"
          >
            Close
          </button>
        </div>
