...
const ChatRowItem: React.FC<{ chat: Chat; visitorId: string }> = ({
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 pb-2.5 border-b border-orange/10 last:border-b-0 last:pb-0 hover:opacity-90 group transition-all"
    >
      {/* Vendor Squircle Avatar */}
      <div className="w-11 h-11 rounded-xl bg-orange/10 text-orange flex items-center justify-center font-bold font-title text-base border border-orange/20 shadow-xs shrink-0 group-hover:scale-105 transition-transform">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1">
          <h4 className="font-title text-xs sm:text-sm font-bold text-gray-900 truncate group-hover:text-orange transition-colors">
            {vendorName}
          </h4>
          {timeAgo && (
